...
class Songs extends Component {
  ...
  _retrieveData = async () => {
    try {
      ...
      team_id = await AStorageManager.getTeamId ();
      console.log ('when retrieve song data', team_id);
      ...
      if (Utils.isNotEmpty (value) && Utils.isNotEmptyOrZero (team_id)) {
        token = value;
  ...
  compare_dates (date1, date2) {
    ...
    if (date1 == 'Invalid date') return 'finished';
    ...
    if (firstDay < secondDay) {
      ...
      if (diffDays > 30) return 'finished';
      ...
      await fetch (api_host + '/user/get/' + decoded.user.id, {
        ...
        .then (response => {
          response.json ().then (data => {
            ...
            var purchase_state = this.compare_dates (
              pDate,
  ...
  render () {
    ...
    return (
      <Container style={styles.mainview}>
        ...
        {limit == 10 && song_length > 9
          ? <View style={{alignItems: 'center'}}>
              <Text style={styles.upgradeText}>
                Your team has more songs! Upgrade your account to see more.
              </Text>
              <TouchableOpacity
        ...
        />
        {this.props.team.isExistTeam
          ? <FloatingButton onPress={this.createSong} />
          : null}
...
export default withNavigationFocus (
  connect (mapStateToProps, mapDispatchToProps) (Songs)
);
