...
import {addToSbt, removeFromSbt} from '../../redux/actions/viewsong';
...
class ViewSong extends Component {
    ...
    onAdd(){
        const {song_id, name, artist, songkey, tempo} = this.state;
        var payload = {
             id: song_id,
             name: name,
             artist: artist,
        ...
        this.props.addToSbt(payload, this.props.team.teamId);
    }

    onRemove(){
        this.props.removeFromSbt(this.state.song_id, this.props.team.teamId);
    }
    ...
    //     console.log(nextProps.navigation.state.params.song_id);
    //     console.log(this.props.navigation.state.params.song_id);
    //     if(nextProps.navigation.state.params.song_id != this.props.navigation.state.params.song_id){
    //         this.setState({
    //             song_id: nextProps.navigation.state.params.song_id,
    //             team_id: nextProps.navigation.state.params.team_id,
    //         })
    //         this._retrieveData();
    //     }
    //   }
    ...
    // if it is not added yet, then return true, or return false added by QQ
    canAdd(){        
        const _sbtList = this.props.sbtList;
        const _sid = this.state.song_id;
        console.log('======>>>>', _sbtList, _sid);
        var flag = true;
        if(_sbtList.length == 0) return true;
        for(var i=0; i< _sbtList.length; i++){
            if(_sbtList[i].id == _sid) flag = false; 
        }
        return flag;
    }
            ...
            <Container style={styles.mainview}>
                ...
                <View style={{ flex: 1 }}>
                    <ScrollView
                        ...
                        {/* </View> */}
                        <View style={{ height: 5 }} />
                         {this.canAdd() ? <AddSbt onPress ={this.onAdd}/> : <RemoveSbt onPress = {this.onRemove}/>} 
                         <View style={{ height: 5 }} />
                        {posts &&
                            posts.map(post => (
                                <Card key={post.id}>
      ...
      >
        <View style={{flexDirection: 'row', alignItems: 'center'}}>
          <MaterialCommunityIcons
            ...
            color={'green'}
          />
          <Text style={{fontSize: Fonts.moderateScale(15), fontWeight: 'normal',color: 'green', fontFamily: Fonts.type.SFUIDisplaySemibold,}}>Add to Songs of Focus</Text>
      ...
      >
        <View style={{flexDirection: 'row', alignItems: 'center'}}>
          <MaterialCommunityIcons
            ...
            color={'#f00'}
          />
          <Text style={{fontSize: Fonts.moderateScale(15), fontWeight: 'normal',color: 'red', fontFamily: Fonts.type.SFUIDisplaySemibold,}}>Remove from the Songs of focus</Text>
...
const mapStateToProps = state => {
  ...
  export default withNavigationFocus(connect (mapStateToProps, mapDispatchToProps) (ViewSong));
