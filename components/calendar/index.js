...
import {Content, Container, Icon, Right, Header, Left, Body} from 'native-base';
...
class Calendar extends Component {
  ...
  _retrieveData = async () => {
    try {
      console.log ('calendar retrieve step');
      var team_id = await AStorageManager.getTeamId ();
  ...
  componentWillUnmount () {
    console.log ('calendar screen unmount....');
    ...
    return (
      <View style={styles.mainview}>
        <Header androidStatusBarColor={'transparent'} style={styles.header}>
          ...
          {
            this.props.team.isExistTeam?<Right style={styles.right}>
            <TouchableOpacity
              onPress={() =>
        ...
        {this.props.team.isExistTeam // isExistTeam;
          ? <ScrollView>
              ...
              {this.state.data < 1
                ? <View style={{flex: 1}}>
                    <Text
                      style={{textAlign: 'center', fontSize: 18, marginTop: 15}}
                    >
                      No Event
                    </Text>
                  </View>
