...
exports.eventPost = functions.https.onCall (data => {
  if (!data.payload) {
    throw new functions.https.HttpsError (
      'invalid-argument', 
  ...
  return sendPostEventNotification (data.payload);
});
      ...
      if (sendUsers.length > 0) {
        querySnapShot.forEach (user => {
          ...
          if (newData.length > 0) {
            let pushToken = user.data ().fcmToken;
            if (pushToken && pushToken !== '') {
              console.log ('when push notification');
...
function sendPostEventNotification (data) {
  const teamId = JSON.parse (data).teamId;
  const content = JSON.parse (data).content;
  const username = JSON.parse (data).username;
  let payload = {
    notification: {
      title: username + ' posted to event',
      body: content,
      sound: 'default',
      badge: '1',
    },
